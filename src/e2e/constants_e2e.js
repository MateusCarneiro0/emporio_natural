...
export const session_username = `TEST_CART1${Math.floor(Math.random() * 10000)}`;
export const session_username_mobile = `TEST_CART_MOBILE${Math.floor(Math.random() * 10000)}`;
...
export const loginUser = async (page, password, mobile) => {
  await page.goto("/");
...
export const addProductInCart = async (page, message, quantity,mobile) => {
  await goToLink(page,"Produtos","",mobile)
